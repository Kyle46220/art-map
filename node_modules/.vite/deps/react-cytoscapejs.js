...
var roundPolygonIntersectLine = function roundPolygonIntersectLine2(x3, y3, basePoints, centerX, centerY, width2, height2, padding, corners) {
  ...
  if (intersections.length > 2) {
    ...
    for (var _i3 = 1; _i3 < intersections.length / 2; _i3++) {
      var squaredDistance = Math.pow(intersections[_i3 * 2] - x3, 2) + Math.pow(intersections[_i3 * 2 + 1] - y3, 2);
...
var fitPolygonToSquare = function fitPolygonToSquare2(points) {
  ...
  for (var _i4 = 0; _i4 < sides; _i4++) {
    x3 = points[2 * _i4] = points[2 * _i4] * sx;
    y3 = points[2 * _i4 + 1] = points[2 * _i4 + 1] * sy;
    minX = Math.min(minX, x3);
    maxX = Math.max(maxX, x3);
    minY = Math.min(minY, y3);
    maxY = Math.max(maxY, y3);
  ...
  if (minY < -1) {
    for (var _i5 = 0; _i5 < sides; _i5++) {
      y3 = points[2 * _i5 + 1] = points[2 * _i5 + 1] + (-1 - minY);
    }
  }
  return points;
};
...
BRp$3.load = function() {
  ...
  var wheelHandler = function wheelHandler2(e2) {
    ...
    if (cy.panningEnabled() && cy.userPanningEnabled() && cy.zoomingEnabled() && cy.userZoomingEnabled()) {
      ...
      cy.emit(e2.type === "gesturechange" ? "pinchzoom" : "scrollzoom");
    }
  };
...
function roundRect(ctx, x3, y3, width2, height2) {
  ...
  ctx.beginPath();
  ctx.moveTo(x3 + radius2, y3);
  ctx.lineTo(x3 + width2 - radius2, y3);
  ctx.quadraticCurveTo(x3 + width2, y3, x3 + width2, y3 + radius2);
  ctx.lineTo(x3 + width2, y3 + height2 - radius2);
  ctx.quadraticCurveTo(x3 + width2, y3 + height2, x3 + width2 - radius2, y3 + height2);
  ctx.lineTo(x3 + radius2, y3 + height2);
  ctx.quadraticCurveTo(x3, y3 + height2, x3, y3 + height2 - radius2);
  ctx.lineTo(x3, y3 + radius2);
  ctx.quadraticCurveTo(x3, y3, x3 + radius2, y3);
  ctx.closePath();
